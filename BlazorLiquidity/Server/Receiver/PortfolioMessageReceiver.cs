...
namespace BlazorLiquidity.Server.Receiver
{
   ...
   public class MessageReceiver : IPortFolioMessageReceiver, IMessageReceiver
   {
      ...
      public async Task Start(string spName, string configName)
      {
         ...
         _messageBroker.SubscribeToTopicSubject(_portfolioName + "." + _configName + "." + Constants.STRATEGY_CONTROL_RESPONSE, ProcessCommands);
         _messageBroker.SubscribeToTopicSubject(_portfolioName + "." + _configName + "." + Constants.STRATEGY_ALIVE_TOPIC, ProcessCommands);
         ...
         using (var stream = new MemoryStream(data))
         {
            ...
            switch (response.ResponseType)
            {
               ...
               case ResponseTypeEnums.GET_BALANCE_RESPONSE_ERROR:
                  ...
                  _logger.LogError("Unsuccessful balance request from {Venue} for instance {Instance} {Error}- re-requesting", response.FromVenue, response.OriginatingSource, balanceError.ErrorMessage);
                  break;

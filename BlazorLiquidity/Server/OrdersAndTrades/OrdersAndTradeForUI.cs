 ...
 ///        {
    ...
    //     }
     ...
     //       _logger.LogError("HandleCancelledOrder - Incorrect state of order {OrderState}", currentState.ToString());
      ...
      {
         _logger.LogInformation($"************************** Got a Trade **************************** on {venue}");
      ...
      public void PlaceOrderUI(OrderDetails placeOrder)
      {
       ...
       //  }
         ...
         MessageBusCommand msg = new MessageBusCommand();
         msg.CommandType = CommandTypesEnum.PLACE_MARKET_ORDER;
         ...
         order.ClientOrderId = placeOrder.ClientOrderId;
         ...
         lock (_locker)
         {
            ...
            foreach (var order in orders)
            {
               OwnOrderChange change = new OwnOrderChange()
               {
                  ...
                  RemainingQuantity = order.Quantity, // TODO
                  Symbol = order.Symbol,
                  Status   = OrderPlacementState.OPEN.ToString(),
         ...
         lock (_locker)
         {
            ...
            try
            {
               ...
               if (orderData == null)
               {
                  ...
                  {
                     var xxx = _tradeTracker[venue];
                     var chosen = xxx.Where(t => t.OrderId.Equals(holder.OrderId));
                     if (chosen.Any())
                     {
                  ...
                  _logger.LogError("OrderPlacementUpdate - no entry for {ClientOid}", holder.OrderId);
                  return;
               }
      ...
      List<OwnOrderChange> IOrderAndTradeProcessing.GetFilledOrders()
      {
         throw new NotImplementedException();
      }
